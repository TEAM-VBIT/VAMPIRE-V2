...
class Inline:
    ...
    def help_markup(
        ...
        if back:
            ...
            emojis = ["👮", "🔐", "🚫", "🌐", "🏓", "🎵", "📜", "📊", "👨‍💻"]
            buttons = [
                self.ikb(text=f"{emojis[i]} {_lang[f'help_{i}']}", callback_data=f"help {cb}")
    ...
    def play_queued(
        ...
        return self.ikm(
            [
                [
                    self.ikb(
                        text=f"⏯ {_text}", callback_data=f"controls force {chat_id} {item_id}"
                    )
                ]
            ]
        )
    ...
    def queue_markup(
        ...
        _emoji = "⏸" if playing else "▶️"
        return self.ikm(
            [[self.ikb(text=f"{_emoji} {_text}", callback_data=f"controls {_action} {chat_id} q")]]
        )
        ...
        return self.ikm(
            [
                [
                    ...
                    self.ikb(text=f"{'👤' if admin_only else '👥'} {admin_only}", callback_data="settings play"),
                ],
                [
                    self.ikb(
                    ...
                    self.ikb(text=f"{'✅' if cmd_delete else '❌'} {cmd_delete}", callback_data="settings delete"),
                ],
                [
                    self.ikb(
    ...
    def start_key(
        ...
        if private:
            rows += [
                [
                    self.ikb(text=f"👤 {lang['aloneowner']}", user_id=config.OWNER_ID),
                    self.ikb(
                        text=f"💻 {lang['source']}",
